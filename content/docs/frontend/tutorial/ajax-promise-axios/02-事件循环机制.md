...
![事件循环流程](./img/event-loop.png)

> **本页关键词**：事件循环、调用栈、宏任务、微任务、Promise、async/await、浏览器与Node.js差异
...
**经典面试题：** 多层嵌套 Promise + setTimeout 的执行顺序，最典型的就是上一节「三、完整示例代码分析」里用三步分析法推导过的 `1 → 6 → 4 → 2 → 3 → 5` 那道题，这里不再重复，重点看下面用日志标签定位执行阶段的调试技巧。
