...
> **本页关键词**：Vite、SFC、Options API、Composition API、setup、ref、reactive、toRefs、computed
...
> **面试要点**：vue-cli 基于 webpack，Vite 基于 ES Module，面试常考两者差异。
...
> **面试要点**：Composition API 按功能组织代码，便于复用与维护；Options API 按选项分散，大组件维护困难。
...
> **面试要点**：ref 在 JS 中访问/修改需用 `.value`，模板中自动解包，无需 `.value`。
...
> **面试要点**：computed 有缓存，依赖不变不重算；与 watch 区别在于 computed 有返回值且可读可写。
