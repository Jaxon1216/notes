...
> ⚠️ 注意：`csurf` 已于 2022 年被官方废弃并归档（不再维护），仅作为了解 CSRF 防护思路的示例。新项目建议改用仍在维护的替代方案，如 [`csrf-csrf`](https://www.npmjs.com/package/csrf-csrf)（Double Submit Cookie 模式）或 [`csrf-sync`](https://www.npmjs.com/package/csrf-sync)（Synchronizer Token 模式）。
