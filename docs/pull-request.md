...
首次参与项目先阅读根目录的 [`CONTRIBUTING.md`](../CONTRIBUTING.md)。协作者默认通过功能分支和 PR 提交改动；仓库管理员可以直接处理低风险的小改，涉及共享功能、路由、构建链路或协作规则时仍建议通过 PR 接受 Review。
...
工作流会检出代码、安装 Node.js 22、通过 `npm ci` 安装锁定依赖，再执行内容结构、图片引用、Vue 标签、单元测试、TypeScript 和生产构建检查。它只负责校验，不负责部署。
...
- 在 PR 描述中填写关联 Issue；没有关联 Issue 时明确写“无”。
- 写明实际运行的检查及结果，未运行的检查说明原因和替代验证方式。
- 明确是否存在破坏性变更或公开 URL 变化；URL 变化需要说明旧地址兼容方式。
- 新增文章要放在合适的信息架构目录下。
- 新增目录时同步补 `meta.json`，并确认侧边栏顺序。
- 新增一级方向或修改导航文案时，先改 `site.config.ts`，再改内容目录。
- 图片放在当前专题附近的 `img/` 目录，正文使用相对路径。
- 资源推荐条目必须包含链接、技术方向、资源类型、推荐理由和标签。
