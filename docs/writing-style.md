...
- 页面截图优先使用 WebP，流程图、架构图和其他矢量图示优先使用 SVG；不要为了格式统一把位图简单包进 SVG。
- 单张图片超过 500 KiB 时应先压缩；超过 1 MiB 会被 `check:images` 阻止提交。
- 不建议直接引用远程图片。优先下载到当前专题的 `img/` 目录；确需保留远程图片时，应确认来源稳定并填写 alt 文案，`check:images` 会给出非阻断警告。
...
- `img/` 中的图片必须至少被一篇 Markdown/MDX 引用，孤立图片会阻止提交。
- 单图超过 500 KiB 时给出压缩警告，超过 1 MiB 时阻止提交。
- 远程图片会提示下载到专题目录，以便使用 Next.js 本地图片优化链路。
